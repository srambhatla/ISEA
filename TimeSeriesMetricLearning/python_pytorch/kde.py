...
class gaussian_kde_my(object):
    ...
    def evaluate(self, points):
        ...
        result = gaussian_kde[spec](self.dataset.T, self.weights[:, None],
...
def someFunction(text1, text2):
    print(text1)
    print(text2)
    print('You passed this Python program '+str(text1) + ' from C! Congratulations!')
    print(np.array(text1))
    return list(text1)



def calc_next_dist(g_init, f, l_lim, u_lim, x_sam):
    '''
    Function takes in the distribution which was used to sample data (g_init), 
    sampled paths x_sam, importance of samples based on target distribution (f) and 
    returns the next distribution to sample from.
    
    
    g_init: initial distribution at the current round
    f: the target distribution we would like to sample from but cannot
    l_lim: lower limit of the seq lengths
    u_lim: upper limit of the sequence length
    x_sam: current samples
    '''
    ...
    kde_pdf = gaussian_kde_my(np.log10(values), 'scott', weights)
    print(kde_pdf)
    ...
    # Bandwidth
    h = 2
    x = np.linspace(l_lim, u_lim, u_lim-l_lim+1)
    
    # Calculate weights
    w = [s*f[s - l_lim]/g_init[s - l_lim] for s in x_sam]
    
    # Calculate its mean
    w_bar = np.mean(w)
    
    # Evaluate next distribution to sample from
    kde_pdf = gaussian_kde_my(x_sam, 'scott', w/w_bar)
    print(kde_pdf)
    logprob = kde_pdf.logpdf(x)
    
    #kde = KernelDensity(kernel='gaussian', bandwidth = h, algorithm='ball_tree').fit(np.expand_dims(x_sam,1))
    #logprob = kde.score_samples(x[:,None])
    
    f_hat = np.exp(logprob)
    print(f_hat)
